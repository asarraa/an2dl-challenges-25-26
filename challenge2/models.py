...
class HistologyResNet(nn.Module):
    def __init__(self, num_classes=4, use_pretrained=True, backbone='resnet18', input_channels=4, freeze_backbone=False):
        """
        Modello basato su ResNet con supporto a 3 o 4 canali.
        
        Args:
            num_classes: Numero di classi in output.
            use_pretrained: Se True, usa pesi ImageNet.
            backbone: 'resnet18' o 'resnet50'.
            input_channels: Canali input (es. 3 per RGB, 4 per RGB+Maschera).
            freeze_backbone: Se True, congela i pesi della parte centrale della rete 
                             e addestra solo il primo layer (conv1) e l'ultimo (fc).
        """
        super().__init__()
        ...
        # ---------------------------------------------------------
        # 2. GESTIONE FREEZING (Prima della chirurgia)
        # ---------------------------------------------------------
        if freeze_backbone:
            for param in self.model.parameters():
                param.requires_grad = False
            print("[INFO] Backbone congelata. Verranno addestrati solo conv1 e fc.")
        ...
        # Se il numero di canali cambia, creiamo un nuovo layer
        if input_channels != original_conv1.in_channels:
            ...
            if use_pretrained:
                with torch.no_grad():
                    # Copia i pesi RGB (primi 3 canali)
        ...
        # IMPORTANTE: Se abbiamo congelato la backbone, assicuriamoci che 
        # il NUOVO conv1 sia addestrabile (per imparare a usare il 4° canale).
        if freeze_backbone:
        ...
        # La testa è nuova, quindi ha requires_grad=True di default.
        # Non serve sbloccarla esplicitamente, ma per sicurezza:
        if freeze_backbone:
    ...
    def unfreeze(self):
        """Metodo helper per sbloccare tutto il modello successivamente"""
