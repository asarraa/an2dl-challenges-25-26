...
def main():
    ...
    # FASE 1: PREPARATION
    # -------------------------------------------------------------------------
    print(">>> FASE 1: Locating Files (Single Core)...")
    
    tasks = [] 
    
    labels_df = pd.read_csv(labels_csv).sort_values(by='sample_index')
    
    # We use a list comprehension here for speed, or a simple loop
    # This part MUST print something if tqdm is working
    for _, row in tqdm(labels_df.iterrows(), total=len(labels_df), desc="Scanning CSV"):
        ...
        # Only do the slow glob if direct path fails
        if not img_path.exists(): 
             # Note: Glob is slow on Drive. Try to avoid it if filenames match exactly.
    ...
    print(f"✅ Found {len(tasks)} valid slides to process.")

    # -------------------------------------------------------------------------
    # FASE 2: EXECUTION
    # -------------------------------------------------------------------------
    if len(tasks) == 0:
        print("🚨 STOPPING: No tasks found. Check your dataset paths.")
        return

    print(f"\n>>> FASE 2: Processing on {os.cpu_count()} CPU Cores...")
    print("    (This might take a moment to start...)")
    
    out_dirs = (out_train_img, out_train_mask, discard_dir, arrays_dir)

    # --- KEY CHANGE: verbose=10 ---
    # This forces joblib to print status updates to the console
    results = Parallel(n_jobs=-1, backend="loky", verbose=10)(
        delayed(process_wrapper)(task, out_dirs) 
        for task in tasks # Note: Removed tqdm here to let joblib handle progress printing
    )
    ...
    train_rows = []
    for res in results:
        if res and res != "SHREK" and isinstance(res, list):
    ...
    if train_rows:
        ...
        print(f"🎉 DONE! Saved {len(train_rows)} patches.")
    else:
