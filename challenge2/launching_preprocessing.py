...
# Default locations (can be overridden via PreprocessingConfig)
DEFAULT_BASE_DATA = Path("../../drive/MyDrive/AN2DL_Challenge2-TheBigBatchTheory/data/dataset")
DEFAULT_OUTPUT_ROOT = DEFAULT_BASE_DATA


@dataclass
class PreprocessingConfig:
    """
    Parametri configurabili per generare versioni diverse del database preprocessato.
    Ogni versione viene salvata dentro output_root/name per non sovrascrivere altre run.
    """
    name: str = "testpreprocessing"
    base_data: Path = DEFAULT_BASE_DATA
    output_root: Path = DEFAULT_OUTPUT_ROOT
    tile_size: int = TILE_SIZE
    stride: int = STRIDE
    hsv_s_thresh: int = HSV_S_THRESH
    hsv_v_thresh: int = HSV_V_THRESH
    background_max_ratio: float = BACKGROUND_MAX_RATIO
...
def process_single_slide(
    ...
    Args:
        ...
        tile_size (int): Dimensione del tile finale (lato). Default: 512.
        stride (int): Passo tra i tile (controlla overlap).
        hsv_s_thresh (int): Threshold su Saturation per distinguere il vetro.
        hsv_v_thresh (int): Threshold su Value per distinguere il vetro.
        background_max_ratio (float): Rapporto massimo di background ammesso.
        downscale_factor (float): Riduzione opzionale dell'immagine prima del tiling.
        max_tiles_per_slide (Optional[int]): Limite superiore ai tile estratti per slide.
        save_arrays (bool): Se True salva anche il corrispondente .npy 4D.
    ...
    for y in range(0, h, stride):
        for x in range(0, w, stride):
            ...
            if is_patch_valid_hsv(
                ...
                tiles_kept += 1
                if max_tiles_per_slide and tiles_kept >= max_tiles_per_slide:
                    break
    ...
    if save_arrays and len(img_array) > 0:
...
# =============================================================================
# 5. MAIN EXECUTION UTILITIES
# =============================================================================

def _prepare_output_dirs(dirs, overwrite=False):
    """
    Crea le cartelle di output evitando di cancellare dati esistenti a meno che
    overwrite non sia esplicitamente abilitato.
    """
    for d in dirs:
        if d.exists():
            if not overwrite:
                raise FileExistsError(
                    f"Cartella {d} già presente. Imposta overwrite=True per rigenerarla."
                )
            shutil.rmtree(d)
        d.mkdir(parents=True, exist_ok=True)


def run_preprocessing(config: PreprocessingConfig, process_test_set: bool = False):
    """
    Preprocessa il dataset secondo la configurazione passata.
    Restituisce un dizionario con le statistiche della run (utile per logging/manifest).
    """
    train_dir = config.base_data / "train_data"
    labels_csv = config.base_data / "train_labels.csv"
    test_dir = config.base_data / "test_data"
    ...
    dirs_to_prepare = [out_train_img, out_train_mask, discard_dir, arrays_dir]
    if process_test_set:
        dirs_to_prepare.extend([out_test_img, out_test_mask])

    _prepare_output_dirs(dirs_to_prepare, overwrite=config.overwrite)
    ...
    # -------------------------------------------------------------------------
    print(f">>> Processing TRAINING SET for version '{config.name}'")
    ...
    if labels_csv.exists() and train_dir.exists():
        ...
        train_rows = []
        stats["slides_total"] = len(labels_df)
        ...
        for _, row in tqdm(labels_df.iterrows(), total=len(labels_df), desc="Training Slides"):
            ...
            res = process_single_slide(
                ...
                hsv_s_thresh=config.hsv_s_thresh,
                hsv_v_thresh=config.hsv_v_thresh,
                background_max_ratio=config.background_max_ratio,
                downscale_factor=config.downscale_factor,
                max_tiles_per_slide=config.max_tiles_per_slide,
                save_arrays=config.save_arrays,
            )
            
            if res == "SHREK":
                stats["slides_shrek"] += 1
        ...
        if train_rows:
            ...
            stats["train_tiles"] = len(train_rows)
    ...
    # -------------------------------------------------------------------------
    # FASE 2: TEST SET PROCESSING (opzionale)
    # -------------------------------------------------------------------------
    if process_test_set and test_dir.exists():
        test_rows = []
...
def main(config: Optional[PreprocessingConfig] = None):
    """
    Esegue il preprocessing con la configurazione di default oppure con quella passata.
    Per evitare cancellazioni involontarie overwrite è False di default.
    """
    cfg = config or PreprocessingConfig()
    run_preprocessing(cfg, process_test_set=False)
